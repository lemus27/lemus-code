...
public class DOMAdapter implements nsIDOMEventListener {
    ...
    {
        final Component source = (Component) chromeAdapter.getWindow();
        ...
        int mods = 0;
        if (nsmev.getShiftKey()) mods |= MouseEvent.SHIFT_DOWN_MASK;
        if (nsmev.getCtrlKey()) mods |= MouseEvent.CTRL_DOWN_MASK;
        if (nsmev.getMetaKey()) mods |= MouseEvent.META_DOWN_MASK;
        if (nsmev.getAltKey()) mods |= MouseEvent.ALT_DOWN_MASK;
        if (nsmev.getButton()==0) mods |= MouseEvent.BUTTON1_DOWN_MASK; //left
        if (nsmev.getButton()==1) mods |= MouseEvent.BUTTON2_DOWN_MASK; //middle
        if (nsmev.getButton()==2) mods |= MouseEvent.BUTTON3_DOWN_MASK; //right
        ...
        final int button;
        if (nsmev.getButton()==0) button = MouseEvent.BUTTON1; //left
        else if (nsmev.getButton()==1) button = MouseEvent.BUTTON2; //middle
        else if (nsmev.getButton()==2) button = MouseEvent.BUTTON3; //right
        ...
        swingAsyncExec(new Runnable() { public void run() {
            try {
                ...
                IMozillaWindow win = (IMozillaWindow) ev.getSource();
                win.onDispatchEvent(ev);
            } catch (Exception e) {
    ...
    private void fireAWTEvent(nsIDOMKeyEvent nskev)
    {
        ...
        int mods = 0;
        if (nskev.getShiftKey()) mods |= KeyEvent.SHIFT_DOWN_MASK;
        if (nskev.getCtrlKey()) mods |= KeyEvent.CTRL_DOWN_MASK;
        if (nskev.getMetaKey()) mods |= KeyEvent.META_DOWN_MASK;
        if (nskev.getAltKey()) mods |= KeyEvent.ALT_DOWN_MASK;
        final int modifiers = mods;
